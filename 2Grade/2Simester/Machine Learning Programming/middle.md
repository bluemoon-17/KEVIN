...
마르코프 과정은 강화학습의 문제를 수학적으로 정의하는 기본 틀입니다.

* **마르코프 속성 (Markov Property):** 가장 중요한 기본 가정입니다. [cite_start]"미래는 오직 현재에만 의존한다"는 뜻으로 [cite: 161][cite_start], 다음 상태($S_{t+1}$)는 오직 현재 상태($S_t$)에만 영향을 받으며, 그 이전의 과거 상태($S_1, ..., S_{t-1}$)와는 관계가 없다는 것입니다[cite: 161].
* [cite_start]**마르코프 과정 (MP):** 마르코프 속성을 따르는 확률 과정입니다[cite: 178]. [cite_start]상태 집합($S$)과 상태 전이 확률($p$)로 구성됩니다 ($\langle S, p, \mu \rangle$)[cite: 178].
* [cite_start]**마르코프 보상 과정 (MRP):** 마르코프 과정(MP)에 **보상($r$)**과 **감가율($\gamma$)**이 추가된 과정입니다[cite: 186].
* [cite_start]**마르코프 결정 과정 (MDP):** 마르코프 보상 과정(MRP)에 **행동($A$)**이 추가된 과정입니다[cite: 191]. [cite_start]강화학습의 문제는 대부분 이 MDP로 표현됩니다[cite: 193].

---

### ✨ 강화학습의 특징

강화학습은 다른 기계학습 방법(지도/비지도)과 구별되는 독특한 특징들을 가집니다.

* [cite_start]**정답 데이터 없음:** 지도 학습처럼 '정답'이 주어지지 않습니다[cite: 163, 167]. [cite_start]대신 **보상(Reward)** 정보가 그와 비슷한 역할을 수행합니다[cite: 163, 167].
* [cite_start]**지연된 보상:** 행동에 대한 보상을 즉시 받지 못하고 한참 시간이 지난 후에 받을 수 있습니다[cite: 163, 167]. [cite_start]이는 학습의 난이도를 높이는 요인이 됩니다[cite: 163, 167].
* [cite_start]**시간 순서 중요:** 강화학습은 타임 스텝(time step)으로 나뉘어진 일련의 상태, 행동, 보상 정보를 순서대로 처리합니다[cite: 163, 167].
* [cite_start]**행동의 영향력:** 에이전트가 수행한 현재의 행동이 다음에 받게 될 상태 정보(미래)에 직접적인 영향을 줍니다[cite: 163, 167].

## 🧭 마르코프 과정 및 기본 요소

---

### 1. 마르코프 과정 (MP)과 확률 과정

| 개념 | 정의 및 구성 요소 |
| :--- | :--- |
| **마르코프 속성** | "미래는 오직 **현재**에만 의존한다"는 가정. 다음 상태($S_{t+1}$)는 오직 현재 상태($S_t$)에만 영향을 받음. |
| **확률 과정** | 마르코프 속성을 따르는 과정. **마르코프 과정 (MP)**이라고 불립니다. |
| **마르코프 과정 (MP)** | 상태 집합($S$)과 **상태 전이 확률**($p$)로 구성됩니다 ($\langle S, p \rangle$). |
...
| 개념 | 정의 |
| :--- | :--- |
| **보상 (Reward, $R$)** | 에이전트의 행동에 대해 환경이 즉시 전달하는 값. |
| **이득 (Return, $G_t$)** | 타임 스텝 $t$ 이후부터 에피소드 종료까지 받게 될 모든 보상의 합. |
| **감가율 ($\gamma$, Gamma)** | 미래 보상의 가치를 할인하는 비율 ($0 \le \gamma \le 1$). 미래 보상보다 현재 보상을 더 가치 있게 만듭니다. |
| **감가 이득** | 감가율을 적용하여 계산된 이득: $G_t = R_{t+1} + \gamma R_{t+2} + \gamma^2 R_{t+3} + ...$ |
| **마르코프 보상 과정 (MRP)** | MP에 **보상($r$)**과 **감가율($\gamma$)**이 추가된 과정입니다. |
...
| 개념 | 정의 및 특징 |
| :--- | :--- |
| **에피소드** | 시작 상태에서 종료 상태까지의 일련의 상호작용. |
| **에피소딕 환경** | 반드시 **종료 상태**가 존재하는 환경 (예: 게임). |
| **지속적 환경** | **종료 상태가 없는** 환경 (예: 로봇 제어, 주식). |

---

### 4. 마르코프 결정 과정 (MDP)

* **마르코프 결정 과정 (MDP, Markov Decision Process):** **강화학습 문제를 정의하는 표준 수학적 모델**입니다.
* MRP에 **행동($A$)**이 추가된 형태입니다.
* **5가지 요소:** 상태 집합($S$), **행동 집합($A$)**, 상태 전이 확률($p$), 보상 함수($r$), 감가율($\gamma$).
