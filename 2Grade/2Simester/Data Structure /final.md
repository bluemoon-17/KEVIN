...
```python
# Push 연산 (Linked Stack)
def push(self, item):
    new_node = Node(item, self.top) # 1. 새 노드를 만들고, 현재 top을 가리킴
    self.top = new_node              # 2. top 포인터를 새 노드로 업데이트
# 복잡도: O(1)
...
for i in range(1, n) : 
    key = A[i] 
    j = i-1 
    # key보다 큰 원소들을 오른쪽으로 한 칸씩 이동 (shift)
    while j >= 0 and A[j] > key : 
        A[j+1] = A[j] # <--- 자료 이동이 일어나는 부분
        j -= 1 
...
def insert_bst(root, node):
    if root is None: # 탐색 실패 지점 (종료 조건)
        return node 
    
    if node.key < root.key:
        # 왼쪽 서브트리에 재귀 호출하고, 결과를 root.left에 갱신
        root.left = insert_bst(root.left, node) 
    else:
        # 오른쪽 서브트리에 재귀 호출하고, 결과를 root.right에 갱신
        root.right = insert_bst(root.right, node)
    return root # 부모 노드로 현재 루트를 반환


# 후계자 노드 찾기 (오른쪽 서브트리의 가장 왼쪽 노드)
def find_successor(node):
    current = node.right # 삭제할 노드의 오른쪽 자식부터 시작
    while current.left is not None:
        current = current.left # 왼쪽 링크를 따라 끝까지 이동
    return current
